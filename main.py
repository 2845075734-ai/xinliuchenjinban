...
def _validate_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # 修复 Bug：兼容 WebUI 传入 List 或 String 的情况
    ...
    async def _delayed_restore_judgment(self, chat_id: str) -> None:
        try:
            ...
            if state.judgment_paused and state.pending_heartflow_reply:
                logger.warning(
                    f"[心流] ⚠️ 判断暂停后台超时，自动恢复判断: {chat_id} "
...
"""
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if res.should_reply:
                ...
                # 修复 Bug：使用 setattr 替代不存在的 set_extra
    ...
    async def on_after_message_sent(self, event: AstrMessageEvent):
        ...
        if getattr(event, "heartflow_triggered", False) and state.pending_heartflow_reply:
            ...
            if has_reply:
                logger.info(
                    f"[心流] ✅ Bot回复完成，已开启沉浸模式并恢复判断: {umo} "
                    f"| 模式: {mode_display}"
                )
            else:
                logger.warning(
                    f"[心流] ⚠️ 心流已触发但Bot无实际回复，仍开启沉浸模式并恢复判断: {umo} "
                    f"| 模式: {mode_display}"
                )
            return
            ...
            if is_command_reply:
                logger.debug(
                    f"[心流] 指令回复不激活沉浸模式: {umo} | {msg_str[:30]}"
                )
                return
            ...
            # 【核心修改】对于普通唤醒，无论是否有实际回复，都开启/刷新沉浸模式
    ...
    async def on_llm_response(self, event: AstrMessageEvent, resp):
        ...
        # 【核心修改】即使 text 为空，也继续执行后续的沉浸模式激活和状态恢复
        if text:
            ...
            logger.warning(
                f"[心流] ⚠️ LLM响应为空，但仍将开启沉浸模式并恢复判断: {umo} "
                f"| 模式: {state.pending_heartflow_mode or 'unknown'}"
            )
        ...
        mode_name = state.pending_heartflow_mode or getattr(event, "heartflow_mode", None) or "unknown"

        self._set_judgment_paused(umo, paused=False)

        logger.info(
            f"[心流] ✅ LLM处理完成，已开启沉浸模式并恢复判断: {umo} "
            f"| 模式: {mode_name} | 是否有实际回复: {'是' if text else '否'}"
        )
...
"""
    ...
    async def terminate(self):
        ...
        logger.info("[心流] 心流插件已卸载，后台恢复任务已清理")
