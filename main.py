...
## 综合判断
阈值: {threshold}（加权总分达到此值才回复）
分数范围: {score_min}-{score_max}分

## 返回格式
严格返回 JSON（注意：所有分数必须在 {score_min} 到 {score_max} 之间，不能为0）：
{{"relevance": {score_min}-{score_max}之间的分数, "willingness": {score_min}-{score_max}之间的分数, "social": {score_min}-{score_max}之间的分数, "timing": {score_min}-{score_max}之间的分数, "continuity": {score_min}-{score_max}之间的分数{reasoning_part}}}"""
...
def _extract_json(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(result, dict):
            return result
        raise ValueError(f"JSON不是对象类型: {type(result)}")
...
def _validate_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if v["dynamic_threshold_min"] > v["dynamic_threshold_max"]:
        logger.warning(f"[心流] 配置警告: dynamic_threshold_min({v['dynamic_threshold_min']}) > dynamic_threshold_max({v['dynamic_threshold_max']})，已自动调整。")
    ...
    try:
        ...
        if len(weights) == 3 and all(0 <= w <= 1 for w in weights):
            total = sum(weights)
            if total > 0:
                v["dynamic_activity_weights"] = [w / total for w in weights]
            else:
...
class HeartflowPlugin(star.Star):
    ...
    def __init__(self, context: star.Context, config: Dict[str, Any]):
        ...
        logger.info(
            ...
            f"基础阈值: {self.reply_threshold} (绝对值: {self.absolute_threshold:.1f}分) | "
    ...
    def _calculate_activity_level(self, chat_id: str) -> float:
        ...
        # 5人以上 = 满分
        diversity_score = min(1.0, sender_count / 5.0)
    ...
    def _get_dynamic_threshold(self, chat_id: str) -> Tuple[float, float, float]:
        ...
        # 根据活跃度插值计算动态阈值
        # activity=0 → 阈值=dynamic_threshold_min（想说话）
        # activity=1 → 阈值=dynamic_threshold_max（不想说话）
        # activity=0.5 → 阈值=base（基准值）
        
        # 使用非线性映射，让中间区域变化更平缓
        # 使用 3次函数：y = base + (x - 0.5) * 3 * (max-min) * (1 - |x-0.5|)
        offset_range = self.dynamic_threshold_max - self.dynamic_threshold_min
        normalized_activity = (activity - 0.5) * 2  # -1 到 1
        ...
        # 限制范围（这里的 min/max 现在会正确使用用户设定的值）
    ...
    def _record_raw_message(self, event: AstrMessageEvent, is_bot: bool = False) -> bool:
        ...
        with self._lock:
            ...
            if buf:
                ...
                if (last_msg.sender_id == ("bot" if is_bot else str(event.get_sender_id())) and
                    last_msg.normalized_content == normalized and 
                    time_diff < 5):
                    return False
    ...
    def _record_bot_reply_manual(self, umo: str, content: str) -> bool:
        ...
        with self._lock:
            ...
            if buf:
                last_msg = buf[-1]
                if (last_msg.is_bot and 
                    last_msg.normalized_content == normalized and 
                    (time.time() - last_msg.timestamp) < 5):
                    return False
    ...
    def _build_chat_context(self, event: AstrMessageEvent) -> str:
        ...
        return f"活跃度:{activity_desc}({activity:.0%}) | 回复率:{rate:.1f}% | 模式:{mode} | 阈值:{dynamic_threshold:.2f} | {datetime.datetime.now().strftime('%H:%M')}"
    ...
    async def judge_with_tiny_model(self, event: AstrMessageEvent) -> JudgeResult:
        ...
        # 获取动态阈值
        dynamic_threshold, activity, absolute_threshold = self._get_dynamic_threshold(event.unified_msg_origin)
        activity_desc = self._get_activity_description(activity)
        ...
        try:
            ...
            logger.error(f"[心流] 提示词模板格式化失败，缺少键: {e}")
        ...
        for attempt in range(self.judge_max_retries + 1):
            try:
                ...
                logger.info(
                    f"[心流] 📊 正常模式: {overall:.3f}/{dynamic_threshold:.2f} "
                    f"(均分: {avg_absolute_score:.1f}分) "
                    f"| {'✅触发' if should_reply else '❌跳过'} | "
                    f"活跃度:{activity_desc}({activity:.0%}) | "
                    f"R:{scores['relevance']:.0f} W:{scores['willingness']:.0f} "
                    f"S:{scores['social']:.0f} T:{scores['timing']:.0f} C:{scores['continuity']:.0f}"
                )
                return JudgeResult(
                    **scores,
                ...
                logger.warning(f"[心流] 正常模式JSON解析失败 (尝试 {attempt+1}/{self.judge_max_retries+1}): {e}")
...
"""
        ...
        for attempt in range(self.judge_max_retries + 1):
            try:
                ...
                logger.info(f"[心流] 🌊 沉浸模式: {'✅触发' if should_reply else '❌跳过'} | 剩余{remaining}秒 | 活跃度:{activity_desc}({activity:.0%}) | {data.get('reasoning', '')}")
                return JudgeResult(
                    should_reply=should_reply,
                    reasoning=data.get("reasoning", ""),
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if res.should_reply:
                ...
                logger.info(f"[心流] 🎯 心流触发回复: {event.message_str[:50]}...")
    ...
    async def on_after_message_sent(self, event: AstrMessageEvent):
        ...
        if has_reply and bot_reply_content:
            recorded = self._record_raw_message(event, is_bot=True)
            if not recorded:
...
基于5个维度评分：相关度、意愿、社交、时机、连贯性。

当评分达到阈值时，会自动触发bot回复并进入
沉浸模式（持续参与对话）。
...
• `enable_heartflow` - 启用心流功能
• `judge_provider_name` - 判断使用的模型
• `reply_threshold` - 基础阈值 (0-1)
• `enable_dynamic_threshold` - 启用动态阈值
• `dynamic_threshold_max` - 动态阈值上限
• `dynamic_threshold_min` - 动态阈值下限
• `dynamic_activity_window` - 活跃度计算窗口
• `immersive_mode_duration` - 沉浸模式时长(秒)
• `judge_prompt` - 自定义评判提示词
