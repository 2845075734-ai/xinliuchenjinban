...
# ============================================================
# 基础提示词模板（不含评分维度描述，由 _build_rules_section 动态填充）
# ============================================================
...
def _build_rules_section(judge_rules: List[Dict[str, Any]]) -> str:
    """
    将 judge_rules 列表转换为提示词中的评分维度描述文本。
    每个维度生成一段 Markdown 格式的低分/高分描述。
    """
    # 确保维度顺序固定
    dimension_order = ["relevance", "willingness", "social", "timing", "continuity"]
    
    # 将配置列表转为以 dimension_name 为键的字典
    ...
    for dim_name in dimension_order:
        ...
        weight = user_rule.get("weight", 0.2)

        lines.append(f"### {idx}. {dim_name} {display}")
        lines.append(f"- 低分情况：{low_desc}")
        lines.append(f"- 高分情况：{high_desc}")
        lines.append("")
        idx += 1
...
def _extract_weights_from_rules(judge_rules: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    if extracted:
        ...
        if total > 0:
            return {k: v / total for k, v in extracted.items() if k in weights}
...
def _validate_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 验证 judge_rules（template_list 格式）
    if "judge_rules" not in v or not isinstance(v["judge_rules"], list):
        v["judge_rules"] = []
    # 确保每个条目都有必要的字段
    cleaned_rules = []
    for item in v["judge_rules"]:
        if isinstance(item, dict) and item.get("dimension_name"):
            cleaned_rules.append({
                ...
                "low_desc": str(item.get("low_desc", "")),
                "high_desc": str(item.get("high_desc", "")),
            })
...
class HeartflowPlugin(star.Star):
    ...
    def _format_chat_history_for_immersive_mode(self, event: AstrMessageEvent) -> str:
        """【修改点1】修复大模型上下文配置问题：使用 context_messages_count 配置，并保证不小于小模型上下文"""
        msgs = self._get_raw_buffer(event.unified_msg_origin)
        if not msgs: return "暂无聊天记录"
        # 使用配置的 context_messages_count，同时保证至少能覆盖小模型判断上下文，避免信息差
        history_count = max(self.context_messages_count, self.judge_context_count)
        history = [f"{'🤖 Bot' if msg.is_bot else f'👤 {msg.sender_name}'}: {self._clean_message_content(msg.content, 150)}" for msg in msgs[-history_count:]]
    ...
    def _build_optimized_contexts(self, raw_msgs: List[RawMessage]) -> List[Dict[str, str]]:
        """【修改点2】同步优化上下文格式化：合并连续同一角色的消息，避免上下文碎片化"""
        contexts = []
        for msg in raw_msgs:
        ...
        # 如果第一条是 bot 消息，转换为 user 角色，避免模型混淆
...
"""
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req):
        ...
        # 【修改点3】防止 Agent 工具调用循环导致的重复注入
        has_context = False
        if hasattr(req, 'extra_user_content_parts') and req.extra_user_content_parts:
            for part in req.extra_user_content_parts:
                if hasattr(part, 'text') and "<group_chat_context>" in str(part.text):
                    has_context = True
                    break
        if has_context:
            return # 已经注入过，直接跳过
