...
class HeartflowPlugin(star.Star):
    ...
    def __init__(self, context: star.Context, config: Dict[str, Any]):
        ...
        # 记录每个群的防抖状态: {"active": bool, "last_reset": float, "latest_msg_str": str, "cancelled": bool}
        ...
        if w_sum <= 0:
            self.weights = {
                "relevance": 0.25, "willingness": 0.2, "social": 0.2, "timing": 0.15, "continuity": 0.2,
            }
    ...
    def _should_process_message(self, event: AstrMessageEvent) -> bool:
        ...
        if event.is_at_or_wake_command:
            ...
            # 如果用户明确@了Bot，且当前群正在防抖，则取消防抖任务
            umo = event.unified_msg_origin
            if umo in self._debounce_states and self._debounce_states[umo].get("active"):
                self._debounce_states[umo]["cancelled"] = True
                logger.debug(f"[心流] 🚫 检测到明确唤醒，取消正在进行的防抖: {umo}")
            return False
    ...
    async def judge_with_tiny_model(self, event: AstrMessageEvent) -> JudgeResult:
        ...
        try:
            judge_prompt = prompt_template.format(
                chat_id=event.unified_msg_origin, minutes_since_reply=self._get_minutes_since_last_reply(event.unified_msg_origin),
                chat_context=self._build_chat_context(event), chat_flow=self._analyze_conversation_flow(event),
                last_bot_reply=self._get_last_bot_reply(event) or "无", sender_name=event.get_sender_name(),
                message=event.message_str, threshold=dynamic_threshold, reasoning_part=reasoning_part,
        ...
        for attempt in range(self.judge_max_retries + 1):
            try:
                ...
                logger.info(f"[心流] 📊 评分模式: {overall:.3f}/{dynamic_threshold:.2f} (均分:{avg_absolute_score:.1f}分) | "
                            f"{'✅触发' if should_reply else '❌跳过'} | 活跃度:{activity_desc}({activity:.0%}) | "
                            f"R:{scores['relevance']:.0f} W:{scores['willingness']:.0f} S:{scores['social']:.0f} "
                            f"T:{scores['timing']:.0f} C:{scores['continuity']:.0f}")

                return JudgeResult(**scores, reasoning=data.get("reasoning", ""), should_reply=should_reply, 
...
## 不应该回复的情况
- 两个用户之间的正常闲聊 / 用户之间的问答 / 简单表情、语气词

## 当前状态
- 模式: 沉浸模式 | 沉浸模式剩余: {remaining}秒 | 群聊活跃度: {activity_desc} ({activity:.0%})
...
"""
        for attempt in range(self.judge_max_retries + 1):
            try:
                resp = await provider.text_chat(prompt=judge_prompt, contexts=[], system=system_inst, image_urls=[])
                data = _extract_json(resp.completion_text.strip())
                should_reply = bool(data.get("should_reply", False))
                logger.info(f"[心流] 🌊 沉浸模式: {'✅触发' if should_reply else '❌跳过'} | 剩余{remaining}秒 | "
                            f"活跃度:{activity_desc}({activity:.0%}) | {data.get('reasoning', '')}")
                return JudgeResult(should_reply=should_reply, reasoning=data.get("reasoning", ""),
                                   confidence=1.0 if should_reply else 0.0, overall_score=1.0 if should_reply else 0.0)
            except (json.JSONDecodeError, ValueError) as e:
        ...
        try:
            ...
            if res.should_reply:
                ...
                if self.enable_debounce:
                    ...
                    # 如果当前群已经在防抖中，说明前一条消息正在等待
                    if debounce_state and debounce_state.get("active"):
                        # 更新最新待回复消息，并重置计时器
                        debounce_state["last_reset"] = time.time()
                        debounce_state["latest_msg_str"] = event.message_str
                        logger.debug(f"[心流] ⏳ 防抖中收到新消息，重置计时器: {umo}")
                        return  # 拦截当前消息，不让它走 LLM
                    
                    # 如果不在防抖中，启动防抖等待
                    self._debounce_states[umo] = {
                        "active": True,
                        "last_reset": time.time(),
                        "latest_msg_str": event.message_str,
                        "cancelled": False
                    }
                    ...
                    while True:
                        # 计算本次需要 sleep 的时间
                        elapsed_since_reset = time.time() - self._debounce_states[umo]["last_reset"]
                        ...
                        if sleep_time <= 0 or self._debounce_states[umo].get("cancelled"):
                            break
                            
                        await asyncio.sleep(min(sleep_time, 0.5))  # 每次最多 sleep 0.5s，以便及时响应取消
                    
                    # 防抖结束，检查是否被取消（比如期间用户@了Bot）
                    if self._debounce_states[umo].get("cancelled"):
                        logger.info(f"[心流] 🚫 防抖被取消（期间有明确唤醒）: {umo}")
                        self._debounce_states.pop(umo, None)
                        return
                    
                    # 取出防抖期间最新的消息内容，更新到当前 event
                    final_msg_str = self._debounce_states[umo]["latest_msg_str"]
                    event.message_str = final_msg_str
                    self._debounce_states.pop(umo, None)
                    logger.info(f"[心流] ✅ 防抖结束，准备回复: {final_msg_str[:30]}...")
                # ================= 防抖逻辑结束 =================
    ...
    async def on_after_message_sent(self, event: AstrMessageEvent):
        ...
        if getattr(event, "heartflow_triggered", False) and state.pending_heartflow_reply:
            ...
            if has_reply:
                logger.info(f"[心流] ✅ Bot回复完成，已开启沉浸模式并恢复判断: {umo} | 模式: {mode_display}")
            else:
                logger.warning(f"[心流] ⚠️ 心流已触发但Bot无实际回复，仍开启沉浸模式并恢复判断: {umo} | 模式: {mode_display}")
            return

        elif event.is_at_or_wake_command:
            msg_str = (event.message_str or "").strip()
            is_command_reply = msg_str.startswith(("/", "#", "!", "！"))
            if is_command_reply:
                return
    ...
    async def on_llm_response(self, event: AstrMessageEvent, resp):
        ...
        umo = event.unified_msg_origin
        state = self._get_chat_state(umo)
        if not state.pending_heartflow_reply:
            return
        ...
        try:
            ...
            logger.debug(f"[心流] on_llm_response解析回复异常: {e}")
