...
"""
    ...
    async def on_after_message_sent(self, event: AstrMessageEvent):
        ...
        if event.get_extra("heartflow_triggered") and state.pending_heartflow_reply:
            mode_name = (
            ...
            if has_reply:
                ...
                self._schedule_pause_restore_task(umo)

                logger.warning(
                    f"[心流] ⚠️ 心流已触发，但未检测到 Bot 实际回复，继续暂停判断: {umo} "
                    f"| 模式: {mode_display} "
                    f"| 将在 {self.judgment_paused_timeout} 秒后由后台任务自动恢复"
                )
    ...
    async def on_llm_response(self, event: AstrMessageEvent, resp):
        ...
        if not text:
            logger.warning(
                f"[心流] ⚠️ LLM响应为空，继续等待发送阶段或后台超时恢复: {umo} "
                f"| 模式: {state.pending_heartflow_mode or 'unknown'}"
            )
            return
        ...
        mode_name = state.pending_heartflow_mode or event.get_extra("heartflow_mode") or "unknown"

        self._set_judgment_paused(umo, paused=False)

        logger.info(
            f"[心流] ✅ LLM已生成回复，提前恢复小模型判断: {umo} "
            f"| 模式: {mode_name}"
        )
